...
function IntegratedSearchCase() {
  return (
    ...
    >
      ...
      <p>
        Next.js Route Handler에서 웨딩홀·업체·스토어 검색 API를 병렬로 호출하고
        하나의 응답으로 조합했습니다. 또한 일부 요청이 실패하더라도 정상적으로
        응답한 영역의 검색 결과는 보여줄 수 있도록 Promise.allSettled를 사용해
        각 요청의 성공과 실패를 개별적으로 처리했습니다.
      </p>
    </CaseBox>
  );
}
...
function HomeCase() {
  return (
    ...
    >
      ...
      <div className={styles["divider"]} />
      <p>
        홈 화면을 어드민에서 구성하고 관리할 수 있도록, 서버에서 전달받은 타입과
        설정에 따라 필요한 컴포넌트를 렌더링하는 구조로 변경했습니다.
      </p>
      <p>
