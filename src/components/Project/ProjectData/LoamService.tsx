...
export default function LoamService() {
  return (
    <ProjectDataContainer
      ...
      description={[
        '- 로스트아크·메이플스토리 캐릭터 정보와 반복적인 플레이 의사결정을 관리하기 위해 개발·운영한 개인용 웹 서비스',
      ]}
      images={[]}
    ...
    >
      <Box sx={{ marginBottom: '8px' }}>
        <ProjectText type='dot'>
          재련 조건·보유 재료·시세를 반영해 기대 비용과 시도별 권장 전략을 계산하는 재련 최적화 기능 개발
        </ProjectText>
      ...
      <Box sx={{ marginBottom: '8px' }}>
        <ProjectText type='dot'>
          입장 레벨, 레이드 선호 조건, 일반·귀속 골드를 기준으로 주간 클리어 골드 전략을 계산·제공
        </ProjectText>
        <ProjectText type='indent'>
      ...
      <Box sx={{ marginBottom: '8px' }}>
        <ProjectText type='dot'>
          외부 로스트아크 API와 캐릭터 데이터를 연결해 원정대 캐릭터 조회·등록, 메인 캐릭터 정렬, 스펙 요약·상세 기능 구현
        </ProjectText>
        <ProjectText type='indent'>
